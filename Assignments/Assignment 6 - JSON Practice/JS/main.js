...
for(var f in data.results.forecast){
   console.log(`Today is ${data.results.forecast[f].day}. The high is ${data.results.forecast[f].high}. The low is ${data.results.forecast[f].low}. It will be ${data.results.forecast[f].text}.`)
}
...
for (let i = 0; i < workers.employees.length; i++){
   console.log(`Name: ${workers.employees[i].firstName} ${workers.employees[i].lastName} Job: ${myJSON.people[i].job}`);
}